...
try {
    ...
    if ($action === 'send' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        if($body==='')throw new RuntimeException('Escribí el contenido del mensaje.');if(unicode_length($subject)>300)throw new RuntimeException('El asunto supera 300 caracteres.');if(strlen($html)>1048576)throw new RuntimeException('El cuerpo supera 1 MB.');$draftRepo=new MailPreferenceRepository((array)config('contacts'));$draftId=(string)($_POST['id']??'');$retained=array_values(array_filter(explode(',',(string)($_POST['draft_attachment_ids']??''))));$attachments=array_merge($draftRepo->draftAttachmentData($email,$draftId,$retained),api_attachments($_FILES['attachments']??[]));if(count($attachments)>10)throw new RuntimeException('Podés adjuntar como máximo 10 archivos.');
