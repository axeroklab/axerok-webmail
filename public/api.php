...
try {
    ...
    if ($action === 'draft') {
        $repo=new MailPreferenceRepository((array)config('contacts'));if($_SERVER['REQUEST_METHOD']==='DELETE'){parse_str((string)file_get_contents('php://input'),$delete);if(!hash_equals((string)$_SESSION['csrf'],(string)($delete['csrf']??'')))json_response(['error'=>'La sesión expiró.'],419);$repo->deleteDraft($email);json_response(['ok'=>true]);}if($_SERVER['REQUEST_METHOD']==='POST'){api_require_csrf();$priority=(string)($_POST['priority']??'normal');if(!in_array($priority,['low','normal','high'],true))$priority='normal';$draft=['to'=>(string)($_POST['to']??''),'cc'=>(string)($_POST['cc']??''),'bcc'=>(string)($_POST['bcc']??''),'subject'=>substr((string)($_POST['subject']??''),0,300),'body_html'=>clean_composer_html((string)($_POST['body_html']??'')),'receipt_requested'=>($_POST['receipt_requested']??'')==='1','priority'=>$priority];if(strlen($draft['body_html'])>1048576)throw new RuntimeException('El borrador supera 1 MB.');$repo->saveDraft($email,$draft);json_response(['ok'=>true]);}json_response(['draft'=>$repo->draft($email)]);
    }
    if ($action === 'folders') {
