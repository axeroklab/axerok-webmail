...
use AxerokMail\Contacts\RoundcubeMysqlReader;
...
function api_attachments(array $files): array
{
    ...
    if(count($files['name'])>20)throw new RuntimeException('Podés adjuntar como máximo 20 archivos.');$result=[];$total=0;
    foreach($files['name'] as $i=>$name){$uploadError=(int)($files['error'][$i]??UPLOAD_ERR_NO_FILE);if($uploadError===UPLOAD_ERR_NO_FILE)continue;if(in_array($uploadError,[UPLOAD_ERR_INI_SIZE,UPLOAD_ERR_FORM_SIZE],true))throw new RuntimeException('Uno de los adjuntos supera el límite de PHP.');if($uploadError!==UPLOAD_ERR_OK)throw new RuntimeException('No se pudo subir uno de los adjuntos.');$size=(int)$files['size'][$i];$total+=$size;if($size>25*1024*1024||$total>50*1024*1024)throw new RuntimeException('Los adjuntos superan el límite permitido.');$tmp=(string)$files['tmp_name'][$i];if(!is_uploaded_file($tmp))throw new RuntimeException('El archivo recibido no es válido.');$data=file_get_contents($tmp);if($data===false)throw new RuntimeException('No se pudo leer un adjunto.');$mime=function_exists('mime_content_type')?mime_content_type($tmp):false;$result[]=['name'=>basename((string)$name),'type'=>(string)($mime?:'application/octet-stream'),'data'=>$data];}
    return $result;
}

/** Config de Roundcube MySQL (fuente unica de contactos/firmas en deploys no-cPanel); null = no configurado. */
function rc_mysql_config(): ?array { $cfg=config('roundcube_mysql'); return (is_array($cfg)&&!empty($cfg['dsn']))?$cfg:null; }
/** Contactos desde la mejor fuente: Roundcube MySQL > Roundcube cPanel > base propia de AxerOK. */
function axerok_contacts_list(string $email): array {
    if(($cfg=rc_mysql_config())!==null){try{return (new RoundcubeMysqlReader($email,$cfg))->contacts();}catch(\Throwable $e){return [];}}
    if(\AxerokMail\Runtime::isCpanel()){try{return (new RoundcubeReader($email))->contacts();}catch(\Throwable $e){return [];}}
    return (new ContactRepository((array)config('contacts')))->all($email);
}
/** Los contactos son de solo lectura (fuente externa Roundcube) cuando hay MySQL o cPanel. */
function axerok_contacts_readonly(): bool { return rc_mysql_config()!==null || \AxerokMail\Runtime::isCpanel(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'login') {
    ...
    try {
        $imapUser=$email;$limiter->assertAllowed($ip,$email);$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$imap->close();$limiter->success($ip,$email);Credentials::store($email,$password,app_credential_key());
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $action === 'sso') {
    ...
    try {
        ...
        // Email limpio como identidad; el login IMAP/SMTP usa el master user.
        Credentials::store($email, $master, app_credential_key(), $email . '*nxsso');
    } catch (Throwable $e) {
...
$email=Credentials::email($requestedAccount);$password=Credentials::password(app_credential_key(),$requestedAccount);$imapUser=Credentials::imapUser($requestedAccount);
if(!$email||!$password){json_response(['error'=>'La sesión expiró.'],401);}
session_write_close();

try {
    if ($action === 'mail-ping') {
        $started=hrtime(true);$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$imap->ping();$imap->close();api_timing('imap',$started);json_response(['ok'=>true,'checked_at'=>time()]);
    }
    if ($action==='attachment'||$action==='inline') {
        $folder=(string)($_GET['folder']??'INBOX');$uid=(int)($_GET['uid']??0);$section=(string)($_GET['section']??'');$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$file=$imap->attachment($folder,$uid,$section);$imap->close();$safe=preg_replace('/[^\pL\pN._ -]/u','_',(string)$file['name'])?:'archivo';header('Content-Type: '.((string)$file['type']?:'application/octet-stream'));header('Content-Length: '.strlen((string)$file['data']));header('Cache-Control: private, max-age=300');header('Content-Disposition: '.($action==='inline'?'inline':'attachment')."; filename*=UTF-8''".rawurlencode($safe));header('X-Content-Type-Options: nosniff');echo $file['data'];exit;
    }
    if ($action==='message-body'&&($_GET['remote']??'')==='1') {
        $folder=(string)($_GET['folder']??'INBOX');$uid=(int)($_GET['uid']??0);$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$message=$imap->message($folder,$uid,false);$imap->close();$resolved=resolve_email_cids((string)$message['html'],(array)($message['inline']??[]),$folder,$uid,$email);header('Content-Type: text/html; charset=utf-8');header("Content-Security-Policy: default-src 'none'; img-src 'self' data: https:; style-src 'unsafe-inline'; font-src data: https:; media-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'self'",true);echo safe_email_html($resolved,true);exit;
    }
    if(in_array($action,['message-headers','message-source','message-download'],true)){
        $folder=(string)($_GET['folder']??'INBOX');$uid=(int)($_GET['uid']??0);$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$contents=$action==='message-headers'?$imap->messageHeaders($folder,$uid):$imap->rawMessage($folder,$uid);$imap->close();
    ...
    if ($action === 'send' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        if(unicode_length($subject)>300)throw new RuntimeException('El asunto supera 300 caracteres.');if(strlen($html)>1048576)throw new RuntimeException('El cuerpo supera 1 MB.');$draftRepo=new MailPreferenceRepository((array)config('contacts'));$draftId=(string)($_POST['id']??'');$retained=array_values(array_filter(explode(',',(string)($_POST['draft_attachment_ids']??''))));$attachments=array_merge($draftRepo->draftAttachmentData($email,$draftId,$retained),api_attachments($_FILES['attachments']??[]));if(count($attachments)>20)throw new RuntimeException('Podés adjuntar como máximo 20 archivos.');if($body===''&&$attachments===[])throw new RuntimeException('Escribí el contenido del mensaje o adjuntá un archivo.');
        $identityRepo=new MailPreferenceRepository((array)config('contacts'));$identity=$identityRepo->preferences($email);$fromEmail=$email;$identityId=trim((string)($_POST['identity_id']??''));if($identityId!==''){$identityFound=false;foreach((($rcm=rc_mysql_config())!==null?(new RoundcubeMysqlReader($email,$rcm))->identities():(\AxerokMail\Runtime::isCpanel()?(new RoundcubeReader($email))->identities():$identityRepo->identities($email))) as $candidate){if((string)$candidate['id']===$identityId){$fromEmail=(string)$candidate['email'];$identity=$candidate;$identityFound=true;break;}}if(!$identityFound)throw new RuntimeException('La identidad elegida no existe.');$bcc=$bcc!==''?$bcc:(string)$identity['default_bcc'];}
        $raw=(new SmtpClient((array)config('mail')))->send($imapUser,$password,$to,$cc,$bcc,$subject,$body,$html,$receipt,$priority,$attachments,(string)($identity['display_name']??''),(string)($identity['reply_to']??''),(string)($identity['organization']??''),$fromEmail);$warning='';
        ...
        try{$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$sent=null;foreach($imap->folders() as $candidate){if($candidate['special']==='sent'){$sent=$candidate['name'];break;}}if($sent!==null)$imap->append($sent,$raw);else$warning='No se encontró la carpeta Enviados.';$imap->close();}catch(Throwable){$warning='El correo salió, pero no se pudo copiar a Enviados.';}
    ...
    if ($action === 'folders') {
        $folders=api_cache_get($email,'folders','all',60);if($folders!==null){header('X-AxerOK-Cache: HIT');json_response(['folders'=>$folders]);}$started=hrtime(true);$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$folders=$imap->folders();$imap->close();api_timing('imap',$started);api_cache_set($email,'folders','all',$folders);header('X-AxerOK-Cache: MISS');json_response(['folders'=>$folders]);
    }
    if ($action === 'folder-create' && $_SERVER['REQUEST_METHOD']==='POST') {
        api_require_csrf();$name=(string)($_POST['name']??'');$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$folders=$imap->folders();$created=false;$firstError=null;
    ...
    if ($action === 'delete-message' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        $imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$trash=null;foreach($imap->folders() as $candidate){if($candidate['special']==='trash'){$trash=$candidate['name'];break;}}if($trash===null)throw new RuntimeException('No se encontró la carpeta Papelera.');if($folder===$trash)$imap->deleteMany($folder,[$uid]);else$imap->move($folder,$uid,$trash);$imap->close();json_response(['ok'=>true,'permanent'=>$folder===$trash]);
    }
    if ($action === 'move-messages' && $_SERVER['REQUEST_METHOD']==='POST') {
        api_require_csrf();$folder=(string)($_POST['folder']??'INBOX');$destination=(string)($_POST['destination']??'');$uids=array_values(array_filter(array_map('intval',explode(',',(string)($_POST['uids']??'')))));$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$valid=false;foreach($imap->folders() as $candidate){if($candidate['name']===$destination&&!in_array('\\Noselect',$candidate['flags']??[],true)){$valid=true;break;}}if(!$valid)throw new RuntimeException('La carpeta de destino no existe.');$imap->moveMany($folder,$uids,$destination);$imap->close();json_response(['ok'=>true]);
    }
    ...
    if ($action === 'delete-messages' && $_SERVER['REQUEST_METHOD']==='POST') {
        api_require_csrf();$folder=(string)($_POST['folder']??'INBOX');$uids=array_values(array_filter(array_map('intval',explode(',',(string)($_POST['uids']??'')))));$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$trash=null;foreach($imap->folders() as $candidate){if($candidate['special']==='trash'){$trash=$candidate['name'];break;}}if($trash===null)throw new RuntimeException('No se encontró la carpeta Papelera.');if($folder===$trash)$imap->deleteMany($folder,$uids);else$imap->moveMany($folder,$uids,$trash);$imap->close();json_response(['ok'=>true,'permanent'=>$folder===$trash]);
    }
    if ($action === 'empty-folder' && $_SERVER['REQUEST_METHOD']==='POST') {
        api_require_csrf();$folder=(string)($_POST['folder']??'');$confirm=(string)($_POST['confirm']??'');$imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$special=null;foreach($imap->folders() as $candidate){if($candidate['name']===$folder){$special=$candidate['special'];break;}}if(!in_array($special,['junk','trash'],true)||!hash_equals('ELIMINAR TODO',$confirm))throw new RuntimeException('La confirmación para vaciar la carpeta no es válida.');$deleted=$imap->emptyFolder($folder);$imap->close();json_response(['ok'=>true,'deleted'=>$deleted]);
    }
    ...
    if ($action === 'contacts-export') {
        $contacts=axerok_contacts_list($email);
        json_response(['filename'=>'axerok-contactos.vcf','contents'=>VCard::export($contacts)]);
    }
    if ($action === 'contacts') { json_response(['contacts'=>axerok_contacts_list($email),'source'=>axerok_contacts_readonly()?'roundcube':'axerok','read_only'=>axerok_contacts_readonly()]); }
    if ($action === 'recent-recipients') {
        $imap=new ImapClient((array)config('mail'));$imap->connect($imapUser,$password);$sent=null;foreach($imap->folders() as $candidate)if($candidate['special']==='sent'){$sent=$candidate['name'];break;}
