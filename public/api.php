...
try {
    ...
    if ($action === 'schedule-send' && $_SERVER['REQUEST_METHOD']==='POST') {
        api_require_csrf();$when=strtotime((string)($_POST['send_at']??''));if($when===false||$when<=time()||$when>time()+366*86400)throw new RuntimeException('La fecha de envío programado no es válida.');$key=trim((string)($_POST['idempotency_key']??''));$payload=['to'=>trim((string)($_POST['to']??'')),'cc'=>trim((string)($_POST['cc']??'')),'bcc'=>trim((string)($_POST['bcc']??'')),'subject'=>trim((string)($_POST['subject']??''))?:'(Sin asunto)','body'=>composer_plain_text(clean_composer_html((string)($_POST['body_html']??''))),'html'=>clean_composer_html((string)($_POST['body_html']??'')),'receipt_requested'=>($_POST['receipt_requested']??'')==='1','priority'=>(string)($_POST['priority']??'normal'),'display_name'=>(string)($_POST['display_name']??''),'reply_to'=>(string)($_POST['reply_to']??''),'organization'=>(string)($_POST['organization']??'')];$queue=new AxerokMailWorkerJobQueue(new PDO((string)config('contacts.dsn'),config('contacts.username'),config('contacts.password'),[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION]));$id=$queue->enqueue($email,'mail.send',$key!==''?$key:bin2hex(random_bytes(16)),$payload,$when);json_response(['ok'=>true,'job_id'=>$id,'send_at'=>date(DATE_ATOM,$when)]);
    }
    ...
    if($action==='identities'){$repo=new MailPreferenceRepository((array)config('contacts'));if(AxerokMailRuntime::isCpanel()){if($_SERVER['REQUEST_METHOD']!=='GET')throw new RuntimeException('En modo cPanel las identidades se administran desde Roundcube.');try{json_response(['identities'=>(new RoundcubeReader($email))->identities(),'source'=>'roundcube','read_only'=>true]);}catch(Throwable){json_response(['identities'=>[],'source'=>'roundcube','read_only'=>true]);}}if($_SERVER['REQUEST_METHOD']==='POST'){api_require_csrf();$identity=['id'=>(string)($_POST['id']??''),'email'=>(string)($_POST['email']??''),'display_name'=>(string)($_POST['display_name']??''),'reply_to'=>(string)($_POST['reply_to']??''),'default_bcc'=>(string)($_POST['default_bcc']??''),'signature_html'=>clean_composer_html((string)($_POST['signature_html']??''))];json_response(['ok'=>true,'identity'=>$repo->saveIdentity($email,$identity),'identities'=>$repo->identities($email)]);}if($_SERVER['REQUEST_METHOD']==='DELETE'){parse_str((string)file_get_contents('php://input'),$delete);if(!hash_equals((string)$_SESSION['csrf'],(string)($delete['csrf']??'')))json_response(['error'=>'La sesión expiró.'],419);$repo->deleteIdentity($email,(string)($delete['id']??''));json_response(['ok'=>true,'identities'=>$repo->identities($email)]);}json_response(['identities'=>$repo->identities($email)]);}
    ...
    if ($action === 'messages') {
        $folder=(string)($_GET['folder']??'INBOX');$page=max(1,(int)($_GET['page']??1));$query=trim((string)($_GET['q']??''));$searchBody=($_GET['body']??'')==='1';$filters=['from'=>(string)($_GET['from']??''),'to'=>(string)($_GET['to']??''),'subject'=>(string)($_GET['subject']??''),'contains'=>(string)($_GET['contains']??''),'exclude'=>(string)($_GET['exclude']??''),'status'=>(string)($_GET['status']??'all'),'size_op'=>(string)($_GET['size_op']??''),'size_bytes'=>max(0,(int)($_GET['size_bytes']??0)),'since'=>(string)($_GET['since']??''),'before'=>(string)($_GET['before']??''),'has_attachment'=>($_GET['has_attachment']??'')==='1'];$global=$folder==='*';$cacheKey='v3|'.$folder.'|'.$page.'|'.hash('sha256',json_encode([$query,$searchBody,$filters],JSON_THROW_ON_ERROR));$fresh=($_GET['fresh']??'')==='1';$payload=$fresh?null:api_cache_get($email,'messages',$cacheKey,8);if(is_array($payload)&&isset($payload['messages'],$payload['total'])){header('X-AxerOK-Cache: HIT');json_response($payload+['folder'=>$folder,'page'=>$page,'query'=>$query,'search_body'=>$searchBody]);}$started=hrtime(true);$imap=new ImapClient((array)config('mail'));$imap->connect($email,$password);$messages=$global?$imap->messagesAcrossFolders($imap->folders(),$page,max(10,min(100,(int)config('mail.page_size',40))),$query,$searchBody,$filters):$imap->messages($folder,$page,max(10,min(100,(int)config('mail.page_size',40))),$query,$searchBody,$filters);$total=$imap->lastMailboxTotal();$imap->close();api_timing('imap',$started);$payload=['messages'=>$messages,'total'=>$total];api_cache_set($email,'messages',$cacheKey,$payload);header('X-AxerOK-Cache: MISS');json_response($payload+['folder'=>$folder,'page'=>$page,'query'=>$query,'search_body'=>$searchBody]);
    }
    if($action==='thread'){
    ...
    if ($action === 'contacts-export') {
        $contacts=(new ContactRepository((array)config('contacts')))->all($email);if(AxerokMailRuntime::isCpanel()){try{$contacts=(new RoundcubeReader($email))->contacts();}catch(Throwable){$contacts=[];}}
        json_response(['filename'=>'axerok-contactos.vcf','contents'=>VCard::export($contacts)]);
    }
    if ($action === 'contacts') { $contacts=(new ContactRepository((array)config('contacts')))->all($email);if(AxerokMailRuntime::isCpanel()){try{$contacts=(new RoundcubeReader($email))->contacts();}catch(Throwable){$contacts=[];}}json_response(['contacts'=>$contacts,'source'=>AxerokMailRuntime::isCpanel()?'roundcube':'axerok','read_only'=>AxerokMailRuntime::isCpanel()]); }
