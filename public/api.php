...
try {
    ...
    if($action==='identities'){$repo=new MailPreferenceRepository((array)config('contacts'));if($_SERVER['REQUEST_METHOD']==='POST'){api_require_csrf();$identity=['id'=>(string)($_POST['id']??''),'email'=>(string)($_POST['email']??''),'display_name'=>(string)($_POST['display_name']??''),'reply_to'=>(string)($_POST['reply_to']??''),'default_bcc'=>(string)($_POST['default_bcc']??''),'signature_html'=>clean_composer_html((string)($_POST['signature_html']??''))];json_response(['ok'=>true,'identity'=>$repo->saveIdentity($email,$identity),'identities'=>$repo->identities($email)]);}if($_SERVER['REQUEST_METHOD']==='DELETE'){parse_str((string)file_get_contents('php://input'),$delete);if(!hash_equals((string)$_SESSION['csrf'],(string)($delete['csrf']??'')))json_response(['error'=>'La sesión expiró.'],419);$repo->deleteIdentity($email,(string)($delete['id']??''));json_response(['ok'=>true,'identities'=>$repo->identities($email)]);}json_response(['identities'=>$repo->identities($email)]);}
