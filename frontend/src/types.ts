export type Session = { authenticated: boolean; email: string | null; accounts:string[];csrf: string;version:string;webmail_home_url?:string|null };
...
export type MailRow = { uid: number;folder:string; from: string; subject: string; date: string; seen: boolean; flagged: boolean;keywords:string[];category?:'principal'|'notification'; size: number };
export type Message = { uid: number; from: string; to: string; cc:string; subject: string; date: string; text: string; html: string };
...
export type Draft = { to:string;cc:string;bcc:string;subject:string;body_html:string;receipt_requested:boolean;priority:'low'|'normal'|'high';updated_at?:string };
