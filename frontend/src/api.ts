...
export const api = {
  ...
  messages: (folder: string, query = '', searchBody=false, fresh=false, filters?:SearchFilters,page=1) => {const size=Number(filters?.size_value||0);const multiplier=filters?.size_unit==='MB'?1048576:1024;return request<{ messages: MailRow[];total:number;unread:number }>('messages', { folder, page:String(page),q: query, ...(searchBody?{body:'1'}:{}), ...(fresh?{fresh:'1'}:{}),...(filters?{from:filters.from,to:filters.to,subject:filters.subject,contains:filters.contains,exclude:filters.exclude,status:filters.status,size_op:filters.size_op,size_bytes:String(Number.isFinite(size)?Math.round(size*multiplier):0),since:filters.since,before:filters.before,...(filters.has_attachment?{has_attachment:'1'}:{})}:{}) });},
  ...
  recentRecipients: () => request<{contacts:Contact[]}>('recent-recipients'),
  ...
  saveDraft: (draft:Draft,csrf:string,files:File[]=[] ) => {const body=new FormData();Object.entries(draft).forEach(([key,value])=>{if(key!=='attachments')body.set(key,value===true?'1':value===false?'0':String(value))});body.set('draft_attachment_ids',(draft.attachments||[]).map(item=>item.id).join(','));files.forEach(file=>body.append('attachments[]',file,file.name));body.set('csrf',csrf);return request<{ok:boolean;draft:Draft}>('draft',{}, {method:'POST',body});},
  ...
  deleteTemplate:(id:string,csrf:string)=>{const body=new FormData();body.set('_operation','delete');body.set('id',id);body.set('csrf',csrf);return request<{ok:boolean;templates:MailTemplate[]}>('templates',{}, {method:'POST',body});},
